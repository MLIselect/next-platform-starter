...
export default function SnowCalculator() {
  ...
  const calculateProbability = (snow, tempMin, wind, rain, country, city, cleanInput, morningIce, sixAmFeels) => {
    ...
    // Wind Chill Trigger (-20°C / -4°F)
    if (sixAmFeels < -20) { bus += 35; school += 10; }
  ...
  const getMessage = (prob) => {
    if (prob >= 99) return { title: "VICTORY: SCHOOL CLOSED! 🚨", mood: "Superintendent surrendered. Go back to sleep." };
      ...
      <div className="p-6 border-b border-slate-700">
        ...
        <div className="flex gap-2 mb-4">
          <input 
            ...
            className="flex-1 bg-slate-900 border border-slate-600 text-white p-4 rounded-lg focus:border-cyan-400 outline-none font-mono uppercase" 
            value={input} 
            onChange={(e) => setInput(e.target.value)} 
            onKeyDown={(e) => e.key === 'Enter' && runPrediction(input)} 
          />
          <button onClick={() => runPrediction(input)} disabled={loading} className="bg-cyan-500 hover:bg-cyan-400 text-slate-900 font-bold py-4 px-6 rounded-lg active:scale-95 transition-transform">{loading ? '⏳' : 'GO'}</button>
        </div>
        {error && <p className="text-red-400 text-xs font-bold mt-2">⚠️ {error}</p>}
      </div>
      ...
      {result && (
        <div className="p-8 bg-gradient-to-b from-slate-800 to-slate-900 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {result.display.bombDetected && (
            <div className="mb-6 bg-red-600/20 border border-red-500 p-4 rounded-xl animate-pulse">
                <div className="flex items-center gap-2 justify-center">
                    ...
                    <h5 className="text-red-400 font-black uppercase text-xs tracking-widest">BOMB CYCLONE ALERT: WEEKEND SENSOR ACTIVE</h5>
                </div>
                <p className="text-white text-[10px] mt-1 font-bold uppercase">Models confirm extreme weekend conditions. Predicted Jan 30-31.</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-slate-900 p-8 rounded-3xl border-2 border-slate-700">
          ...
          <button onClick={tweetResult} className="mt-8 w-full py-4 bg-sky-500 hover:bg-sky-400 text-white rounded-xl font-black uppercase tracking-widest transition-all shadow-lg active:scale-95">🐦 Tweet Your Prediction</button>
        </div>
      )}
    </div>
  );
